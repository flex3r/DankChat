...
class RootViewDeferringInsetsCallback(
    ...
    val ignorePersistentInsetTypes: () -> Boolean = { false },
    ...
    override fun onApplyWindowInsets(
        ...
        val ignorePersistentTypes = ignorePersistentInsetTypes()
        val persistentOrZero = persistentInsetTypes.takeIf { deferredInsets || !ignorePersistentTypes } ?: 0
        val deferredOrZero = deferredInsetTypes.takeUnless { deferredInsets } ?: 0
        val types = persistentOrZero or deferredOrZero

        // Finally we apply the resolved insets by setting them as padding
        val typeInsets = windowInsets.getInsets(types)
        v.updatePadding(
            left = typeInsets.left.takeUnless { deferredInsets && ignorePersistentTypes } ?: v.paddingLeft,
            right = typeInsets.right.takeUnless { deferredInsets && ignorePersistentTypes } ?: v.paddingRight,
            top = typeInsets.top.takeUnless { deferredInsets && ignorePersistentTypes } ?: v.paddingTop,
