...
// 🍓🍑🍊🍋🍍NaM forsenE 🍐🍏🐬🐳NaM forsenE
// test NaM🙅🏻‍♂️NaM Keepo Keepo NaM🙅🏻‍♂️NaM Keepo 🙅🏻‍♂️ Keepo NaM🙅🏻‍♂️ Keepo 🙅🏻‍♂️NaM Keepo NaM🙅🏻‍♂️🙅🏻‍♂️🙅🏻‍♂️NaM
...
fun String.appendSpacesBetweenEmojiGroup(): Pair<String, List<Int>> {
    ...
    var index = 0
    var prevEmojiIndex = -1
    matches.forEachIndexed { idx, (value, range) ->
        // handle substrings at the start of the message up to the first emoji, or in-between emojis
        if (index < range.first && index > prevEmojiIndex) {
            val prev = substring(range = index..<range.first)
            // if not at the start of the message and the first char of the substring is no whitespace, append one
            if (idx > 0 && !prev.first().isWhitespace()) {
                addedSpacesPositions += index
                fixedContentBuilder.append(" ")
            }

            fixedContentBuilder.append(prev)
            index += prev.length

            // if the end of the substring is no whitespace, append one
            if (!fixedContentBuilder.last().isWhitespace()) {
                addedSpacesPositions += index
                fixedContentBuilder.append(" ")
            }
        }
        ...
        // append the emoji
        fixedContentBuilder.append(value)
        index += value.length
        prevEmojiIndex = range.last
    }
    ...
    if (index < lastIndex) {
        ...
        fixedContentBuilder.append(end)
    }
